...
- **原文层**：**必须一字不差地引用 corpus 中的原始文本**（建议引用毛泽东、王阳明与曾国藩各自 3-5 句话的段落，也就是每次回答至少要10-15条原文作为支撑，切忌只摘录孤立的一两句或用自己的话概括）。以体现深入的调研与扎实的文献基础。使用 Markdown 的 blockquote (`>`) 格式进行展示，并标注标题、作者、来源 URL。
